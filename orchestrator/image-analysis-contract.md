  ...
  "objects": [
    {
      ...
      "count_estimate": 3,
      "estimated_size_m": { "height": 0.6, "width": 0.35, "length": 0.35 },
      "estimated_mass_kg": 4,
...
- `estimated_size_m`: estimated real-world dimensions in meters of a single instance of the object (not the group, not the on-screen size). `height` is required and load-bearing; `width` and `length` are optional but recommended (they spread objects out so they don't overlap). Estimate from world knowledge plus visible cues: a person ≈ 1.8 m, an ISO shipping container ≈ 2.6 m tall, a gantry/port crane ≈ 40–70 m, a coffee mug ≈ 0.1 m, a car ≈ 1.5 m. The frontend uses `height` to scale each mesh to true metric proportions, so a crane towers over a container. When genuinely unsure, give your best physical estimate rather than omitting it.
- `estimated_mass_kg`: estimated real-world mass in kilograms of a single instance, accounting for it being a hollow/real object (not a solid block). Examples: a coffee mug ≈ 0.3 kg, a chair ≈ 6 kg, a person ≈ 80 kg, a car ≈ 1500 kg, an empty ISO container ≈ 2300 kg, a port crane ≈ 800000–2000000 kg, a loaded container ship ≈ 100000000+ kg. The frontend uses this as the physics mass so heavy objects resist being pushed. Optional; when omitted the physics engine falls back to a density-based estimate from the collider volume.
