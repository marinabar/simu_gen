...
Carry `estimated_size_m` and `estimated_mass_kg` from the approved object candidate in `image.json` into each `object.json`; `height` (meters) is required and is used by the frontend to scale the mesh to true real-world proportions, and `estimated_mass_kg` sets the physics weight. See `IMAGE-BLAST.md` field guidance for how to estimate them.
