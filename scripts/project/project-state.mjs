...
const PROJECT_DIRS = ['source', 'output', 'output/world']
const RESERVED_OUTPUT_DIRS = new Set(['world'])
const MODEL_EXTENSIONS = new Set(['.glb', '.obj', '.fbx', '.usdz'])
const STAGE_EXTENSIONS = new Set(['.avif', '.gif', '.heic', '.heif', '.jpeg', '.jpg', '.png', '.webp'])
...
async function scanObjects(worldDir) {
  ...
  for (const entry of entries) {
    ...
    const hasModel = objectFiles.some(
      (filePath) => isVisibleFile(filePath) && MODEL_EXTENSIONS.has(path.extname(filePath).toLowerCase()),
    )
    objects.push({
...
export async function ensureProjectState(options) {
  const { slug: rawSlug, description, displayName, write = true, stageInput = false, inputDir = 'input' } = options
  ...
  return {
    ...
    state: {
      ...
      has_world_operation:
        worldOutputFiles.some(isWorldRequest) ||
        (await pathExists(path.join(worldOutputPath, 'operation.json'))),
