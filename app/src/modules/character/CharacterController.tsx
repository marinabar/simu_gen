...
export const CharacterController = forwardRef<CharacterControllerHandle>(
  ...
  useImperativeHandle(ref, () => ({ reset: resetToSpawn }), [resetToSpawn])
  ...
  }, [gl.domElement, resetToSpawn])

  useFrame((_state, delta) => {
    const body = bodyRef.current
    if (!body) return

    // Safety: if the player falls out of the world (e.g. spawned inside geometry
    // and ejected, or walked off an edge), snap back to spawn.
